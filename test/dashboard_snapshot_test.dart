...
void main() {
  ...
  group('total card', () {
    testWidgets('shows the total balance in the green card', (tester) async {
      final state = await _freshState();
      ...
      // The headline balance now lives in the green Total card (a 40px figure
      // under a "Total balance" label), not the old collapsible hero.
      expect(find.text('Total balance'), findsOneWidget);
      final total = find.byWidgetPredicate(
        (w) => w is Text && w.data == 'TSh 250,000' && w.style?.fontSize == 40,
      );
